...
int main(int argc, const char *argv[])
{
  ...
  std::shared_ptr<TestBehavior_SECC_SDP> tb = std::make_shared<TestBehavior_SECC_SDP>(mtc, stc);
  std::shared_ptr<HAL_61851_Listener> v_HAL_61851_Listener;
  cfg->f_SECC_CMN_PR_InitConfiguration_001(v_HAL_61851_Listener, stc);
  pre->f_SECC_CMN_PR_StateB_001(v_HAL_61851_Listener);
  for (uint8_t i = 0; i < 5; i++) {
    tb->f_SECC_CMN_TB_VTB_SDP_001(0x10, fail);
  }
  std::cin.get();
